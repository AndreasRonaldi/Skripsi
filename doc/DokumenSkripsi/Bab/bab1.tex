...
\textit{Online Judge} yang digunakan oleh UNPAR dinamakan SharIF-Judge~\cite{stillmen:sharif} yang merupakan hasil dimodifikasi oleh Stillmen Vallian terhadap Sharif-Judge~\cite{javed:sharif} buatan Mohammad Javad Naderi yang dibuat menggunakan \textit{framework} CodeIgniter dan Bash. Gambar~\ref{fig:1:dashboardpng} merupakan tampilan halaman utama setelah memasuki situs web SharIF-Judge dalam \textit{browser} yaitu halaman \textit{Dashboard}.
...
\begin{enumerate}
    ...
    \item Bagaimana menganalisa data rekaman pemutaran ulang mahasiswa?
\end{enumerate}
...
\begin{enumerate}
    ...
    \item Menganalisa data rekaman hasil eksperimen mengenai pola yang muncul dengan menvisualisasikan data menjadi bagan-bagan seperti \textit{histogram}. 
\end{enumerate}
...
\begin{itemize}
    ...
    \item Pengujian perangkat lunak dilakukan pada dalam sistem tertutup di mana akan  diawasi.
\end{itemize}
...
\begin{enumerate}
    ...
    \item Menganalisa hasil eksperimen.
...
\begin{itemize}
    ...
    \item \textbf{Bab 5:} Implementasi dan Pengujian \\
          Membahas implementasi fitur pada SharIF Judge dan pengujian yang dilakukan beserta dengan analisa hasil pengujian.
